...
const AdvocatePage = () => {
  ...
  const fetchAdvocates = useCallback(async (currentPage = 1) => {
    setLoading(true);
    setError('');
    try {
      ...
      const res = await api.get(`/advocates/getAll?${params}`);
  ...
  useEffect(() => {
    queueMicrotask(() => fetchAdvocates(page));
  }, [fetchAdvocates, page]);
  ...
  const handleSubmit = async e => {
    ...
    try {
      await api.post('/advocates/upload', data);
      setSubmitMsg('success');
      setFormData(emptyForm);
      setSelectedFile(null);
      if (fileInputRef.current) fileInputRef.current.value = null;
      fetchAdvocates(1); setPage(1);
    } catch (err) {
  ...
  const handleBulkUpload = async () => {
    ...
    setBulkMessage('');
    try {
      const res = await api.post('/advocates/bulk-upload', data, {
  ...
  return (
    <div className="register-page">
      ...
      <div className="tab-nav">
        ...
        <button className={`tab-btn${tab === 'records' ? ' active' : ''}`} onClick={() => { setTab('records'); fetchAdvocates(page); }}>
          View Records
        </button>
      ...
      {tab === 'add' && (
        <div className="tab-panel">
          <p className="panel-desc">Fill in the details below and attach the relevant document to create a new advocate hearing entry.</p>
          {submitMsg === 'success' && (
          ...
          <form onSubmit={handleSubmit} className="entry-form" noValidate>
            ...
            <div className="form-row two-col">
              ...
              <div className="field">
                <label>Petitioner Name <span className="req">*</span></label>
                <input type="text" name="petitioner_name" value={formData.petitioner_name} onChange={handleChange} placeholder="Full name" required />
              </div>
            </div>
            <div className="form-row two-col">
              <div className="field">
                  ...
                  <tbody>
                    {advocates.map(a => (
                      <tr key={a.serial_no}>
                        ...
                        <td className="mono">{a.hearing_date}</td>
                        <td className="mono">{a.case_no}</td>
                        <td>{a.court_hall_no}</td>
                        <td>{a.petitioner_name}</td>
                        <td>{a.counsel_name_through_vc}</td>
                        <td>{a.technical_person}</td>
                        <td>
                          {a.uploaded_file
                            ? <span className="file-badge">{a.uploaded_file.split('/').pop()}</span>
      ...
      {tab === 'import' && (
        <div className="tab-panel">
          ...
          <div className="import-info">
            ...
            <ul>
              <li>Date</li><li>Case No</li><li>Court Hall No</li>
              <li>Petitioner Name <em>(or Petitonar Name)</em></li>
